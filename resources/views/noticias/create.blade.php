    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <form action="{{ route('noticias.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="mb-4">
                ...
                <select name="role_id" id="role_id" class="w-full border border-gray-300 rounded-lg p-2" >
...
<x-app-layout>
    <div class="container mx-auto px-4 py-6">
        ...
        <form action="{{ route('noticias.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-6">
            ...
            <div class="mb-4">
                <label for="role_ids" class="block text-gray-700 font-semibold mb-2">Roles</label>
                <select name="role_ids[]" id="role_ids" class="w-full border border-gray-300 rounded-lg p-2" multiple >
                    @foreach ($roles as $role)
                        <option value="{{ $role->id }}">{{ $role->name }}</option>
                    @endforeach
                </select>
